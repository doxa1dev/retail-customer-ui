...
import { REASONS_FOR_CUSTOMER_TO_NOT_HOST } from '../../../../core/constants/constant';
import { isNullOrUndefined } from 'util';
import { Title } from 'app/core/enum/title';
@Component({
  selector: 'app-customer-no-host',
  templateUrl: './customer-no-host.component.html',
  styleUrls: ['./customer-no-host.component.scss']
})
export class CustomerNoHostComponent implements OnInit {
  title = Title.LEFT;
  order: Order;
  reason: string;
  reasonInput: string;
  reasonList = REASONS_FOR_CUSTOMER_TO_NOT_HOST;
  ...
  backHistory()
  {
    ...
    this.orderSevice.updateReasonForNoHostNoUnbox(rea, this.order_id, 1).subscribe(
      data =>
      {
        this.router.navigate(['/order-history'], { state: { selectTab: 6 } });
      });
